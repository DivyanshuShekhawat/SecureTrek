...
export function FileUploader({ onUpload, progress, maxFileSize = 100 * 1024 * 1024 }: FileUploaderProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <div className="w-4 h-4 sm:w-5 sm:h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />;
    }
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      >
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className={`mx-auto w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center transition-colors duration-300 ${
      ...
      {progress.length > 0 && (
        <div className="space-y-3">
          <h4 className="font-medium text-gray-900 dark:text-white text-sm sm:text-base">Upload Progress</h4>
          {progress.map((item, index) => (
            <div key={index} className="bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-3 sm:p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                  <File className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 dark:text-gray-500 flex-shrink-0" />
